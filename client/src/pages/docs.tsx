...
import { Copy, Play, CheckCircle, ArrowRightLeft } from "lucide-react";
...
def get_swap_quote(input_amount, input_mint, output_mint, slippage=0.5):
    ...
    if response.status_code == 200:
        ...
        input_readable = int(quote['inputAmount']) / 1e9  # SOL
        output_readable = int(quote['outputAmount']) / 1e6  # USDC
        rate = output_readable / input_readable
        
        print(f"Swapping {input_readable} SOL for {output_readable:.2f} USDC")
...
for token in high_volume_tokens:
    ...
    print(f"{token['symbol']}: ${token['price']:.2f} (${market_cap_billions:.1f}B market cap)")`,
      nodejs: `const axios = require('axios');
...
const analyzeTokens = async () => {
  try {
    ...
    volumeRatios.slice(0, 5).forEach((token, index) => {
      console.log((index + 1) + '. ' + token.symbol + ': ' + (token.volumeToMarketCapRatio * 100).toFixed(2) + '% daily volume ratio');
    });
...
def monitor_api_health():
    ...
    print(f"Error Rate: {metrics['errorRate']}%")
...
export default function DocsPage() {
  ...
  const currentEndpoint = apiEndpoints.find(e => e.id === activeEndpoint) || apiEndpoints[0];
